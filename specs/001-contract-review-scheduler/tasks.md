...
| 層級 | 技術棧 | 負責範圍 |
|------|--------|---------|
| **後端** | ASP.NET Core 8.0, SQL Server, EF Core | REST API, 業務邏輯, 資料庫 |
| **前端** | React 19.2.0, TypeScript, Bootstrap | UI/UX, 表單, 月曆, 狀態管理 |
| **通訊** | REST API (JSON) | HTTP/HTTPS via Axios |
...
**第 1-2 周** (基礎設施 - 並行執行)
- **後端**: Task 1.1-1.5 (基礎設施, 7.5 小時)
- **前端**: Task 0.1-0.4 (基礎設施, 8.5 小時)
- 目標: 後端和前端框架都可運行

**第 2-3 周** (認證系統 - 並行執行)
- **後端**: Task 2.1-2.5 (認證授權, 12 小時)
- **前端**: Task A.1-A.2 (登入 + 表單, 5 小時)
- 目標: 端到端登入流程可運作

**第 3-5 周** (核心功能 - 並行執行)
- **後端**: Task 3.1-3.5 (預約核心, 12 小時) + Task 5.1-5.5 (郵件, 9 小時)
- **前端**: Task A.3-A.6 (預約 UI, 10 小時)
- 目標: 預約功能完整

**第 5 周** (審查人員功能 - 並行執行)
- **後端**: Task 4.1-4.5 (審查人員, 9 小時)
- **前端**: Task B 系列 (進階功能, 12 小時)
- 目標: 審查人員完整工作流

**第 6-7 周** (測試 + 部署)
- **後端**: Task 6.1-6.6 (測試, 19 小時)
- **後端**: Task 7.1-7.3 (文件部署, 7 小時)
- 目標: 全面測試和部署準備
...
- **後端開發者**: 1 名 (ASP.NET Core + SQL Server)
- **前端開發者**: 1 名 (React + TypeScript + Bootstrap)
- **QA 工程師**: 1 名 (可選)
- **基礎設施**:
  - SQL Server 2019+ 或 Express
  - SMTP 郵件伺服器存取
  - AD/WiAD 伺服器存取
  - 開發環境 (VS 2022, VS Code, Node.js 18+)
